 ...
 * @author Luke Taylor
 * @author Dave Syer
 * @author 庄金明 修改了 createAccessToken refreshAccessToken 方法 (only update createAccessToken refreshAccessToken method)
 * 修改后实现：
 * 1 每次登录都刷新了 accessToken，并且加满了过期时间，原 accessToken n 秒内仍有效
 * 2 用 refreshToken 刷新 accessToken，原 accessToken n 秒内仍有效
 */
public class ZjmzxfzhlTokenServices implements AuthorizationServerTokenServices, ResourceServerTokenServices, ConsumerTokenServices, InitializingBean {
    ...
    /**
     * 原token缓冲 n 秒仍有效，防止高并发出现大量401
     */
    private int oldAccessTokenValiditySeconds = 10;

    public ZjmzxfzhlTokenServices() {
    }

    public ZjmzxfzhlTokenServices(int oldAccessTokenValiditySeconds) {
        if (oldAccessTokenValiditySeconds > 0) {
            this.oldAccessTokenValiditySeconds = oldAccessTokenValiditySeconds;
        }
    }
    ...
    public OAuth2AccessToken createAccessToken(OAuth2Authentication authentication) throws AuthenticationException {
        ...
        if (existingAccessToken != null) {
            ...
            tokenStore.storeAccessToken(existingDefaultOAuth2AccessToken, authentication);
        }

        // 2 新建新的token和refreshToken
