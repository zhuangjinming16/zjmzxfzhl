...
public class CreateController {
	...
	public static void create(String createTableName, String templetPath) {
		StringBuffer stringBufferAll = new StringBuffer();// 所有Controller代码
		...
		// 读取模板内容
		File actionFile = new File(CodeUtil.projectPath + "src\\main\\java\\com\\zjmzxfzhl\\codeCreate\\template\\" + templetPath + "\\Controller.txt");
		try {
		...
		try {
			String filePath = CodeUtil.projectPath + "src\\main\\java\\com\\zjmzxfzhl\\modules\\" + _sys + "\\" + "\\controller\\";
			File file = new File(filePath);
			file.mkdirs();
			BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter(new File(filePath + _Sys + _CodeType + "Controller.java")));
	...
	public static void main(String[] args) {
		CreateController.create(CodeUtil.createTableName, "01CRUDE_I");
	}
}
