...
public class CreateVue {
	...
	public static void create(String createTableName, String templetPath) {
		...
		String searchConditions = "", searchConditionsData = "", searchColumnsList = "", primaryKey = "", allColumnsDialog = "", allColumnsData = "",
				allColumnsRules = "", loadDicts = "";
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if ("createBy".equals(codeTypeId) || "createDate".equals(codeTypeId) || "createTime".equals(codeTypeId) || "updateBy".equals(codeTypeId)
		...
		writeFile(stringBufferAll, sysUpper, sys, aSys, aCodeType, codeType, searchConditions, searchConditionsData, searchColumnsList, primaryKey,
				allColumnsDialog, allColumnsData, allColumnsRules, loadDicts, tableNameCn);
	}

	private static void writeFile(StringBuffer stringBufferAll, String sysUpper, String sys, String aSys, String aCodeType, String codeType,
			String searchConditions, String searchConditionsData, String searchColumnsList, String primaryKey, String allColumnsDialog,
			String allColumnsData, String allColumnsRules, String loadDicts, String tableNameCn) {
		try {
	...
	private static String resolveSearchColumnsList(String searchColumnsList, TableObject tableObject, String codeTypeId) {
		if (StringUtils.isNotEmpty(tableObject.getCodeTypeId())) {
			searchColumnsList += "            <el-table-column label=\"" + tableObject.getColumnNameCn() + "\" prop=\"" + codeTypeId
					+ "\" align=\"center\"><template slot-scope=\"scope\"><span v-html=\"formatDictText(dicts." + tableObject.getCodeTypeId()
					+ ",scope.row." + codeTypeId + ")\"></span></template></el-table-column>" + "\r\n";
		} else {
			searchColumnsList += "            <el-table-column label=\"" + tableObject.getColumnNameCn() + "\" prop=\"" + codeTypeId
					+ "\" align=\"center\"><template slot-scope=\"scope\"><span>{{ scope.row." + codeTypeId
	...
	private static String resolveSearchConditions(String searchConditions, TableObject tableObject, String codeTypeId) {
		if (StringUtils.isNotEmpty(tableObject.getCodeTypeId())) {
			searchConditions += "            <el-select v-model=\"listQuery." + codeTypeId + "\" placeholder=\"" + tableObject.getColumnNameCn()
					+ "\" class=\"filter-item\"><el-option v-for=\"(item, index) in dicts." + tableObject.getCodeTypeId()
			...
			if (DATE_TYPE.equals(tableObject.getDataType())) {
				...
				searchConditions += "            <el-input v-model=\"listQuery." + codeTypeId + "\" placeholder=\"" + tableObject.getColumnNameCn()
